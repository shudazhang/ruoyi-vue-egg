...
module.exports = (app) => {
  const { DATE, STRING, BIGINT, CHAR, INTEGER } = app.Sequelize
  const GenTableColumn = app.model.define('gen_table_column', {
    ...
    javaField: {
      ...
      comment: 'JAVA字段名',
      set(javaField) {
        if (['undefined', 'null', ''].includes('' + javaField)) {
          throw new Error('Java属性不能为空')
        }
        this.setDataValue('javaField', javaField)
      }
    },
